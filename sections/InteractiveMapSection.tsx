...
export function InteractiveMapSection() {
  ...
  return (
    <section id="live-map" className="px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1440px] space-y-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-10%' }} variants={fadeUp}>
          <SectionHeading
            eyebrow="Section 04 - Interactive map demo"
            title="A living map, not a static list."
            description="Explore nearby events, watch markers respond to attention, and open previews without losing your sense of place."
          />
        </motion.div>
          ...
          <div className="overflow-hidden rounded-[2.4rem] border border-[var(--line)] bg-[var(--surface-strong)] shadow-[0_28px_90px_rgba(17,24,39,0.14)]">
            <div className="relative h-[520px]">
              ...
              <div className="pointer-events-none absolute left-4 right-4 top-4 flex items-start justify-between gap-3">
                <Card className="surface-card-strong pointer-events-auto max-w-md rounded-[1.7rem] p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--secondary)]">Map demo</p>
                  <p className="mt-2 text-sm leading-6 text-muted">
                    {loading
                      ? 'Looking for nearby activity and loading the strongest local signals.'
                      : `${events.length} events surfaced. Hover a marker or tap a card to preview it.`}
                  </p>
                </Card>
            ...
            {events.slice(0, 3).map((event) => {
              ...
              return (
                <motion.button
                  ...
                  className="block w-full rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-5 text-left shadow-[0_22px_50px_rgba(17,24,39,0.1)]"
                >
